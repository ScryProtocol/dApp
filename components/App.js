...
let token = new ethers.Contract(
  ...
  const fetchCommissions = async () => {
    try {
      if (contract) {
        const commission = await contract.connect(signer).getUserCommissions(account);
  ...
  const fetchAllCommissions = async () => {
    if (contract) {
      const numCommissions = await contract.connect(signer).nextRequestId();
      const fetchedCommissions = [];
      for (let i = 0; i < numCommissions; i++) {
        const commission = await contract.connect(signer).allCommissions(i);
  ...
  const startCommission = async (requestId) => {
    ...
    try {
      ...
      const updatedCommission = await contract.connect(signer).allCommissions(requestId);
  ...
  const fetchArtistProfile = async (artistAddress) => {
    try {
      ...
      setCommissions(await contract.connect(signer).getArtistCommissions(artistAddress));
      setUserCommissions(await contract.connect(signer).getUserCommissions(artistAddress));
      setArtistProfile(await contract.connect(signer).getArtistProfile(artistAddress));
...
<h3 >{artistProfile.bio}</h3></pre>
  ...
  }} href={'https://chat.blockscan.com/index?a='+selectedArtist}><button style={{ marginLeft: '10px', alignSelf: 'center', backgroundColor:'#23acff'}
              } >Message</button></a>
            </div>
          }        {comms == 1 && (
            ...
            <section id="nft-bounty-details">
              ...
              <div className="card" style={{ padding: '42px' }}>
                <div className="nft-viewer">
                  ...
                  }} href={'https://chat.blockscan.com/index?a='+selectedCommission.commissioner}><button style={{ marginLeft: '6px',backgroundColor:'#23acff'}
                              } >Message</button></a>
                    <p>
