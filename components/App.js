...
const bgColors = [
  'bg-blue-300  bg-opacity-20 backdrop-blur-lg',
  'bg-red-300 bg-opacity-20 backdrop-blur-lg',
  'bg-green-300 bg-opacity-20 backdrop-blur-lg',
  'bg-yellow-300 bg-opacity-20 backdrop-blur-lg',
  'bg-pink-300 bg-opacity-20 backdrop-blur-lg',
  'bg-purple-300 bg-opacity-20 backdrop-blur-lg',
  'bg-indigo-300 bg-opacity-20 backdrop-blur-lg',
];
...
const handleCancelTransaction = async (txIndex) => {
  ...
  const [showVaultOnly, setShowVaultOnly] = useState(true);
          ...
          <div className="tab-switcher mb-4 flex justify-center space-x-4">
            <div
              className={`tab px-4 py-2 rounded-full cursor-pointer ${
                activeTab === 'open' ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-400'
              }`}
            ...
            <div
              className={`tab px-4 py-2 rounded-full cursor-pointer ${
                activeTab === 'create' ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-400'
              }`}
            ...
            <div
              className={`tab px-4 py-2 rounded-full cursor-pointer ${
                activeTab === 'settings' ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-400'
              }`}
      ...
      // OpenVaultSection Component
      const OpenVaultSection = () => {
        return (
          <div className="space-y-6">
            <div>
              <label htmlFor="vault-search" className="block mb-2 font-semibold text-gray-400">
                Search Vault:
              </label>
              <input
                type="text"
                id="vault-search"
                name="vault-search"
                className="w-full p-3 bg-gray-700 bg-opacity-50 border-none rounded-full focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out"
                placeholder="Enter vault name to search"
              />
            </div>
            <button
              className="w-full py-3 bg-blue-500 text-white font-semibold rounded-full hover:bg-blue-700 transition duration-300 ease-in-out"
      ...
      // CreateVaultSection Component
      const CreateVaultSection = () => {
        return (
          <div className="space-y-6 relative">
            <div>
              <label htmlFor="vault-name" className="block mb-2 font-semibold text-gray-400">
                Vault Name:
              </label>
              <button
              ...
              <div className="flex items-center">
                <input
                  ...
                  name="vault-name"
                  required
                  className="w-full p-3 bg-gray-700 border-none rounded-l-full focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out"
                  placeholder="Enter vault name"
                />
                <span className="bg-gray-700 bg-opacity-50 p-3 rounded-r-full text-gray-400">.vlt.eth</span>
              </div>
            </div>
      
            <div>
              <label htmlFor="recovery" className="block mb-2 font-semibold text-gray-400">
                Recovery Address:
              </label>
              <input
                type="text"
                id="recovery"
                name="recovery"
                placeholder="Enter address. This address is for backup and should be a cold wallet, has full control."
                required
                className="w-full p-3 bg-gray-700 bg-opacity-50 border-none rounded-full focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out"
              />
            </div>
            <div>
              <label htmlFor="custom-whitelist" className="block mb-2 font-semibold text-gray-400">
            ...
            <div>
              <label htmlFor="transaction-delay" className="block mb-2 font-semibold text-gray-400">
            ...
            <div>
              <label htmlFor="threshold" className="block mb-2 font-semibold text-gray-400">
                Threshold:
              </label>
              <input
                type="number"
            ...
            <div>
              <label htmlFor="custom-limits" className="block mb-2 font-semibold text-gray-400">
      ...
      const SettingsSection = () => {
        return (
          <div className="space-y-6">
            <h1 className="text-pink-500 text-center text-lg font-semibold">Info</h1>
            <div>
              <label htmlFor="vault-name" className="block mb-2 font-semibold text-gray-400">
                Owner Address:
              </label>
              <p className="text-gray-200 bg-gray-700 bg-opacity-50 rounded-3xl text-center overflow-hidden">
                {vaultSettings.owner}
              </p>
            </div>
            <div>
            ...
            <div className="space-x-6 flex items-center justify-center">
              <div>
                <label htmlFor="daily-limit" className="block mb-2 font-semibold text-gray-400">
                  Daily Limit:
                </label>
                <p className="text-gray-200 text-center bg-gray-700 bg-opacity-50 rounded-3xl">
              ...
              <div>
                <label htmlFor="delay" className="block mb-2 font-semibold text-gray-400">
                  Delay:
                </label>
                <p className="text-gray-200 w-40 text-center bg-gray-700 bg-opacity-50 rounded-3xl">
                  D:{(vaultSettings.delay / 84000).toFixed(0)} H:{((vaultSettings.delay % 84000) / 3600).toFixed(0)} M:{((vaultSettings.delay % 3600) / 60).toFixed(0)} S:{(vaultSettings.delay % 60).toFixed(0)}
                </p>
              </div>
            </div>
            <div>
              <label htmlFor="whitelisted-addresses" className="block mb-2 font-semibold text-gray-400">
            ...
            {vaultSettings.whitelistedAddresses && vaultSettings.whitelistedAddresses.length > 0 ? (
              vaultSettings.whitelistedAddresses.map((address, index) => (
                <p key={index} className="text-gray-200 text-center bg-gray-700 bg-opacity-50 rounded-3xl m-0 overflow-hidden">
                  {address}
                </p>
              ))
            ) : (
              <p className="text-gray-200">No whitelisted addresses found.</p>
            )}
            <h1 className="text-pink-500 text-center text-lg m-2 font-semibold">Update</h1>
            <div className="flex items-center justify-center space-x-4">
              <div>
                <label htmlFor="withdraw-limit" className="block mb-2 font-semibold text-gray-400">
                  Limit Per Day of An Asset (%):
                </label>
                <input
                  type="number"
            ...
            <div>
              <label htmlFor="recovery-addresses" className="block mb-2 font-semibold text-gray-400">
                Recovery Addresses:
              </label>
              <input
                type="text"
            ...
            {(userAddress === vaultSettings.owner ||
              ...
              (vaultSettings.whitelistedAddresses && vaultSettings.whitelistedAddresses.includes(userAddress))) && (
              <button
                className="w-full py-3 bg-blue-500 text-white font-semibold rounded-full hover:bg-blue-700 transition duration-300 ease-in-out"
                onClick={() => {
      ...
      const displayTransactions = () => {
        return queuedTransactions.map((transaction, index) => (
          <div
            key={transaction.id}
            className={`${bgColors[index % bgColors.length]} text-gray-200 grid grid-cols-1 sm:grid-cols-5 text-center rounded-3xl p-4 mb-4 shadow-lg transition-transform transform hover:scale-105`}
          >
            <div className="flex items-center justify-center sm:justify-left space-x-4 mb-2 sm:mb-0 lg:relative lg:right-20">
              <div
                className={`${
                  transaction.executed ? 'bg-blue-500' : 'bg-red-500'
                } text-white text-lg rounded-full p-2 relative sm:right-4`}
              >
                {transaction.to !== selectedVault ? (
                  <img
                    className="h-6 w-6"
                    src={
                      tokenLogos[transaction.token.toLowerCase()]
                        ? tokenLogos[transaction.token.toLowerCase()]
            ...
            <a href={'https://etherscan.io/address/' + transaction.to}>
              <div className="text-pink-400 font-semibold text-left text-center relative lg:right-20 lg:top-2">
            ...
            <div className="flex items-center relative lg:top-2">
              <div
                className={`${
                  transaction.executed
                    ? transaction.numConfirmations === 404
                      ? 'text-red-100 bg-red-500'
                      : 'text-green-100 bg-green-500'
                    : 'text-yellow-100 bg-yellow-500'
                } rounded-full px-3 font-bold mb-2 mx-auto`}
              >
                {transaction.executed
                  ? transaction.numConfirmations === 404
                    ? 'Canceled'
              ...
              {!transaction.executed && userAddress === vaultSettings.owner && (
                <button
                  className="bg-red-500 text-white font-semibold relative left-1 py-1 px-3 rounded-full hover:bg-orange-500 transition duration-300 ease-in-out ml-2 bottom-1"
  ...
  return (
    <>
      ...
      {tokenBalances.map((asset, index) => (
        <div
          key={asset.symbol}
          className={`${bgColors[index % bgColors.length]} px-4 py-2 md:px-6 md:py-4 rounded-3xl shadow-lg flex flex-col items-center text-white relative space-y-2`}
        >
          {/* Settings Gear Icon */}
          <div
            className="gear-icon text-lg absolute top-3 right-3 cursor-pointer text-white"
            onClick={() => {
              handleLimitModalToggle();
              setSelectedToken(asset.address);
            }}
            title="Adjust Asset Limits"
          >
            ⚙️
          </div>
          ...
          <div className="w-full flex flex-col md:flex-row justify-between items-center text-center space-y-2 md:space-x-6 font-semibold overflow-hidden">
            ...
            <div className="w-full md:w-1/2 text-md">
              ...
              <div className="rounded-full py-2 px-3 bg-orange-400">
                {convert(asset.wallet).toString().substring(0, 12)}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center w-full space-y-2 mt-4">
            <input
              ...
              min="0"
              step="0.01"
              className="w-full lg:w-1/4 md:w-full mt-2 text-center text-white rounded-full p-2 text-sm bg-gray-500 bg-opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out mx-auto"
              placeholder="Amount"
              value={depositAmount[asset.address] || ''}
              onChange={(e) => setDepositAmount({ ...depositAmount, [asset.address]: e.target.value })}
            />
            <button
              className="w-full lg:w-1/4 md:w-full bg-blue-500 text-white font-semibold py-2 px-4 rounded-full text-sm hover:bg-gray-200 transition duration-300 ease-in-out mx-auto"
              style={{ minWidth: '80px' }}
              onClick={() =>
                handleDepositToken(
                  asset.address,
                  depositAmount[asset.address] || '0'
                )
              }
      ...
      {nftAssets.map((nft, index) => (
        ...
        >
          ...
          <div
            ...
            onClick={() => {
              handleLimitModalToggle();
              setSelectedToken(nft.contract.address);
            }}
            title="Adjust NFT Limits"
          >
            ⚙️
          </div>
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
            <button
              className="bg-blue-500 text-white font-semibold py-2 px-4 rounded-full hover:bg-gray-200 transition duration-300 ease-in-out mt-4 text-sm"
              onClick={() => handleWithdrawNft(nft)}
              title={`Withdraw ${nft.name}`}
            >
              Withdraw
            </button>
          </div>
          ...
          {todeposit.map((asset, index) => (
            ...
            >
              ...
              <input
                ...
                min="0"
                step="0.01"
                className="rounded-full text-center text-white p-2 text-sm bg-gray-500 bg-opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out"
                placeholder="Amount"
                value={depositAmount[asset.address] || ''}
                onChange={(e) => setDepositAmount({ ...depositAmount, [asset.address]: e.target.value })}
              />
              ...
              <button
                className="bg-blue-500 text-white font-semibold py-2 px-4 rounded-full hover:bg-gray-200 transition duration-300 ease-in-out text-sm"
                onClick={() =>
                  handleDepositToken(
                    asset.address,
                    depositAmount[asset.address] || '0'
  ...
  function DepositModal({ handleClose, handleDepositToken }) {
    ...
    return (
      <div className="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={handleClose}>
        <div style={{width:'400px'}}className="modal-content bg-gray-900 bg-opacity-95 p-8 rounded-3xl shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
          <span className="close cursor-pointer text-white text-2xl absolute top-4 right-4" onClick={handleClose}>&times;</span>
      ...
      <div className="tab-switcher mb-4 flex justify-center space-x-4">
        ...
        <div className={`tab ${!nft ? 'tab-active' : ''}`} onClick={() => setNft(0)}>Token</div>
            ...
            {!nft && (<>
              <div>
                <label htmlFor="token" className="block mb-2 font-semibold text-white">Token Address:</label>
                <select id="token" name="token" required className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setSelectedToken(e.target.value)}>
                ...
                {selectedToken === 'custom' &&
                  <input type="text" id="customToken" name="customToken" className="w-full p-3 mt-2 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" placeholder="Enter custom token address" onChange={(e) => { setSelectedToken(e.target.value); toast.success('Token set') }} />}
              </div>
              <div className="flex space-x-4">
                <div className="flex-1">
                  <label htmlFor="amount" className="block mb-2 font-semibold text-white">Amount:</label>
                  <input type="number" id="amount" name="amount" step="0.01" required className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={handleAmountChange} />
                </div>
              </div>
  ...
  {nft==1 && (<>
              <div>
                <label htmlFor="nft" className="block mb-2 font-semibold text-white">NFT Address:</label>
                <input type="text" id="nft" name="nft" required className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setSelectedToken(e.target.value)} />
                <label htmlFor="tokenId" className="block mb-2 font-semibold text-white">Token ID:</label>
                <input type="text" id="tokenId" name="tokenId" required className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setAmount(e.target.value)} />
  ...
  function LimitModal({ handleClose }) {
    return (
      <div className="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={handleClose}>
        <div className="modal-content bg-gray-900 bg-opacity-90 p-8 rounded-3xl shadow-2xl relative" onClick={e => e.stopPropagation()}>
          <span className="close cursor-pointer text-white text-2xl absolute top-4 right-4" onClick={handleClose}>&times;</span>
          <section id="limit-settings">
            ...
            <div className="tab-switcher mb-4 flex justify-center space-x-4">
              <div className={`tab px-4 py-2 rounded-full cursor-pointer ${limitSection === 'fixed' ? 'bg-pink-500 text-white' : 'bg-gray-800 text-white'}`} onClick={() => setLimitSection('fixed')}>Fixed Limit</div>
              <div className={`tab px-4 py-2 rounded-full cursor-pointer ${limitSection === 'percentage' ? 'bg-pink-500 text-white' : 'bg-gray-800 text-white'}`} onClick={() => setLimitSection('percentage')}>Percentage Limit (%)</div>
              <div className={`tab px-4 py-2 rounded-full cursor-pointer ${limitSection === 'no-limit' ? 'bg-pink-500 text-white' : 'bg-gray-800 text-white'}`} onClick={() => setLimitSection('no-limit')}>No Specific Limit</div>
            </div>
            {limitSection === 'fixed' && (
              <div id="fixed-limit-section" className="space-y-6">
                <div>
                  <label htmlFor="fixed-limit" className="block mb-2 font-semibold text-white">Fixed Limit:</label>
                  <input type="number" value={tokenLimit.fixedLimit} id="fixed-limit" name="fixed-limit" step="1" className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setTokenLimit({ ...tokenLimit, fixedLimit: e.target.value })} />
                </div>
              </div>
            )}
            {limitSection === 'percentage' && (
              <div id="percentage-limit-section" className="space-y-6">
                <div>
                  <label htmlFor="percentage-limit" className="block mb-2 font-semibold text-white">Percentage Limit (%):</label>
                  <input type="number" value={tokenLimit.percentageLimit} id="percentage-limit" name="percentage-limit" step="0.01" className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setTokenLimit({ ...tokenLimit, percentageLimit: e.target.value })} />
                </div>
              </div>
            )}
            {limitSection === 'no-limit' && (
              <div id="no-limit-section" className="space-y-6">
                <div className="text-center text-white font-semibold">No limit set for this token.</div>
                <select id="use-base-limits" value={tokenLimit.useBaseLimit} name="use-base-limits" className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setTokenLimit({ ...tokenLimit, useBaseLimit: e.target.value })}>
...
<div className="space-y-8 mt-12">
  ...
  function CustomTxModal({ handleClose, customTx, setCustomTx, handleSendCustomTx }) {
    ...
    return (
      <div className="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={handleClose}>
        <div className="modal-content bg-gray-900 bg-opacity-95 p-8 rounded-3xl shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
          <span className="close cursor-pointer text-white text-2xl absolute top-4 right-4" onClick={handleClose}>&times;</span>
          <section id="custom-tx">
            ...
            <div className="space-y-6">
              <div>
                <label htmlFor="to" className="block mb-2 font-semibold text-white">To Address:</label>
                <input type="text" id="to" name="to" value={customTx.to} className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setCustomTx({ ...customTx, to: e.target.value })} />
              </div>
              <div>
                <label htmlFor="value" className="block mb-2 font-semibold text-white">ETH Value:</label>
                <input type="number" id="value" name="value" step="0.01" value={customTx.value} className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setCustomTx({ ...customTx, value: e.target.value })} />
              </div>
              <div>
                <label htmlFor="fnSig" className="block mb-2 font-semibold text-white">Function Signature:</label>
                <select id="fnSig" name="fnSig" value={customTx.fnSig} className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => setCustomTx({ ...customTx, fnSig: e.target.value, params: Array(e.target.value.split(',').length - 1).fill('') })}>
                ...
                {customTx.fnSig === 'custom' &&
                  <input type="text" id="customFnSig" name="customFnSig" className="w-full p-3 mt-2 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" placeholder="Enter custom function signature" onChange={(e) => setCustomTx({ ...customTx, fnSig: e.target.value, params: Array(e.target.value.split(',').length - 1).fill('') })} />}
              </div>
              {paramInputs.map((param, index) => (
                <div key={index}>
                  <label htmlFor={`param${index}`} className="block mb-2 font-semibold text-white">{param.split(' ')[1] || `param${index}`}:</label>
                  <input type="text" id={`param${index}`} name={`param${index}`} value={customTx.params[index] || ''} className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => handleParamChange(index, e.target.value)} />
                </div>
              ))}
              {customTx.fnSig === 'calldata' &&
              (<div>    
                  <label className="block mb-2 font-semibold text-white">Enter call data</label>
                  <input type="text" value={customTx.params || ''} className="w-full p-3 bg-gray-800 border-none rounded-full focus:ring-2 focus:ring-pink-500 transition duration-300 ease-in-out" onChange={(e) => handleParamChange(0, e.target.value)} />
                </div>
              )}
...
return (
  <div className="min-h-screen bg-gradient-to-r from-gray-900  via-gray-800 to-gray-900 text-gray-200">
    ...
    <main className="mx-auto py-8 px-4 sm:px-8">
      <Header />
      <section id="vault-management" className="bg-gray-700 bg-opacity-40 p-8 rounded-3xl shadow-2xl mb-8 lg:w-1/2 mx-auto">
        <TabSwitcher activeTab={currentTab} onTabChange={handleTabChange} />
        {currentTab === 'open' && (
          <>
            <OpenVaultSection />
            <div className="mt-2">
              <label htmlFor="vault-select" className="block mb-2 font-semibold text-gray-400">
                My Vaults:
              </label>
              <select
                id="vault-select"
      ...
      <div className="text-center items-center">
        <h1 className="inline-block bg-gray-700 bg-opacity-50 rounded-3xl text-center text-white mx-auto font-bold mb-8 px-2 text-xs sm:text-base md:text-base xl:text-base">
          {selectedVault}
        </h1>
      </div>
      <section id="vault-assets" className="bg-gray-700 bg-opacity-40 p-8 rounded-3xl shadow-2xl mb-8">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl text-pink-500 font-bold">Assets in Vault</h2>
          <div className="space-x-2">
            <button
              className={`px-4 py-2 rounded-full font-semibold transition duration-300 ease-in-out text-white ${
                showVaultOnly ? 'bg-blue-500 hover:bg-blue-700' : 'bg-pink-500 hover:bg-pink-700'
              }`}
              onClick={() => {
                setShowVaultOnly(!showVaultOnly);
                fetchDeps();
              }}
            >
            ...
            <button
              className="bg-pink-500 text-white font-semibold py-2 px-4 my-6 rounded-full hover:bg-pink-700 transition duration-300 ease-in-out"
      ...
      <section id="allowance-list" className="mt-8">
        <div className="bg-gray-700 bg-opacity-40 p-8 rounded-3xl shadow-2xl overflow-x-auto">
          <button
            className="bg-pink-500 text-white font-semibold lg:py-2 px-4 relative bottom-14 lg:bottom-0 rounded-full hover:bg-pink-700 transition duration-300 ease-in-out float-right"
            onClick={handleCustomTxModalToggle}
          >
            Queue Custom Transaction
          </button>
          <h2 className="text-xl text-pink-500 font-bold mb-4">Vault Transactions</h2>
          <div className="grid grid-cols-5 text-center font-semibold text-gray-400 mb-4">
