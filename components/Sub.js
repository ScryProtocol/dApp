...
let streamContractAddress = '0x27d632e1525a965dadd960a80c40fe8b31f8ea47';
...
const[acc, setAccounts] = useState(null);
  ...
  const checkSubscription = async () => {
    ...
    if (token && subscribe && amount && window && once) {
      ...
      setSubscriptionDetails({lender: '', friend: subscribe, token: token, totalStreamed: 0, outstanding: 0, allowable: amount, window: once == 'false' ? `Pay every ${Math.floor(Number(window) / 86400)}d:${Math.floor((Number(window) % 86400) / 3600)}h:${Math.floor((Number(window) % 3600) / 60)}m:${Number(window) % 60}s` : `Sub until ${new Date(Date.now() + Number(window) * 1000).toLocaleString()}`, timestamp: '', once: once,});
...
subscribe = await provider._getAddress(subscribe);
    ...
    if (await tokenContract.balanceOf(userAddress) < (ethers.parseUnits(amount, dec))) {toast.error('You do not have enough tokens.'); return;}
