...
let provider
const contractAddress = '0x470e00b6D001B005d20a702a005705000014F700';
...
function App() {
  ...
  useEffect(() => {
    async function init() { await setfee( await contract.fee())
console.log(contract)
let b = await contract.connect(signer).checkBalance(address, token,network)
      setBal(Number(b[0]) / 10 ** 18);
      setfeedID           (Number(await contract.connect(signer).userBalanceFeed(network, await address, token)));
 await setfee( await contract.fee())
      let feedValue
  ...
  const sync = async (e) => {
    ...
    let b = (await contract.checkBalance(await address, e.target.value,network));
    setBal(Number(b[0]) / 10 ** 18);
  ...
  async function lol() {
    ...
    setBal(Number(b[0]) / 10 ** 18);
    console.log(network, addrs, token, alt, 'LOL', Number(await contract.checkBalance(addrs, token,network)) / 10 ** 18)
