...
const contractAddress = '0x7985f6eCAdf368FFB9C8E7D4a1E2f6BF7c858a75';
...
function App() {
  ...
  const [addrs, setaddrs] = useState('0x0000000000071821e8033345a7be174647be0706');
  const [ID, setID] = useState('0');
  const [IPFS, setIPFS] = useState(null); const [OracleReady, setOracleReady] = useState(0);
  ...
  useEffect(() => {
    ...
    console.log(urls);
      ...
      setsigs(urls)
      console.log(sigs)} window.ethereum.request({ method: 'eth_requestAccounts' });
    provider = new ethers.providers.Web3Provider(window.ethereum);
        ...
        <div ><a href='https://scry.finance' ><img style={{ height: '42px', position: 'fixed' }} src="/scry.png" className="top-2 right-56 color-white border-white" />
          ...
          <div style={{ color: '#ffffff', backgroundColor: '#53baff', position: 'relative', top: '10px', borderRadius: '25px' }} className="justify-center text-center flex flex-col bg-gray-800 space-y-6 justify-center m-auto max-w-4xl min-w-80 shadow-md rounded-md border border-solid border-white overflow-hidden">
            ...
            <Select
              ...
              <MenuItem value="0x0000000000071821e8033345a7be174647be0706">Ethereum</MenuItem>
            ...
            </div>Please note there is a 0.001 ETH oracle fee.</div >
        <div>
        </div>
      </div >
    );
