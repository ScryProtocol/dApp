...
contract Delta is ERC20 {
    ...
    function mint(address to, uint amount) public {
        ...
        IERC20(collateralToken).transferFrom(
            msg.sender,
            address(this),
            (amount * strikePrice) / 10 ** priceDec
        );
        collateral[msg.sender] += (amount * strikePrice) / 10 ** priceDec;
        _mint(to, amount);
    ...
    function getDelta() public view returns (uint profit) {
        if (strikePrice > currentPrice) {
            uint payoff = (strikePrice - currentPrice);
            profit = payoff;
        }
        return profit;
    }
    ...
    function unlock(uint amount) public {
        ...
        _burn(msg.sender, amount / (strikePrice / 10 ** priceDec));
