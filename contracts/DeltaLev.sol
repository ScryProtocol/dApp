...
constructor()ERC20('',''){}
    ...
    function mint(address to, uint amount) public {
        ...
        collateral[to] += amount;
        _mint(to, amount);
    }

    function getDelta() public view returns (uint profit) {
        if (strikePrice > currentPrice) {
            uint payoff = (strikePrice - currentPrice) * lev < strikePrice
                ? (strikePrice - currentPrice) * lev
                : strikePrice;
            profit = payoff;
        }
        return profit;
    ...
    function unlock(uint amount) public {
        uint holderPayoff = collateral[msg.sender] -
    ...
    function redeem() public {
        ...
        if (strikePrice > currentPrice) {
            uint payoff = (strikePrice - currentPrice) * lev < strikePrice
                ? (strikePrice - currentPrice) * lev
                : strikePrice;
            uint holderPayoff = (payoff * balanceOf(msg.sender)) / strikePrice;
            uint holderCollateral = collateral[msg.sender] * 
                (strikePrice-getDelta())/strikePrice;
    ...
    function updateFeeds(uint256[] memory bounties) external payable {
        require(
